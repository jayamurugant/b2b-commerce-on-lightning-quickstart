#!/usr/bin/env python3
"""FirstCry stock monitor v3.

Built on top of v1/v2 behavior with one key enhancement:
- Recheck-before-notify mode (default ON):
  when an alert condition is detected, wait a random delay (default 10-20s),
  fetch fresh data again, and only notify if the alert still holds.

This reduces transient alerts where a product briefly appears in stock/listing.
"""
...
def build_compact_alert_text(
    ...
    lines: List[str] = [
        f"{args.email_subject_prefix} v3",
        f"Run #{run_no}",
        (
    ...
    for recipient in args.twilio_recipients:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "firstcry-stock-monitor-v3",
            },
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="FirstCry stock monitor v3 with recheck-before-notify."
    )
    ...
    # Recheck-before-notify controls
    parser.add_argument(
        "--verify-before-alert",
        dest="verify_before_alert",
        action="store_true",
        help="Enable second validation fetch before sending notifications (default ON)",
    )
    parser.add_argument(
        "--no-verify-before-alert",
        dest="verify_before_alert",
        action="store_false",
        help="Disable second validation fetch before sending notifications",
    )
    parser.set_defaults(verify_before_alert=True)
    parser.add_argument(
        "--verify-delay-min",
        type=int,
        default=10,
        help="Minimum seconds before verification fetch",
    )
    parser.add_argument(
        "--verify-delay-max",
        type=int,
        default=20,
        help="Maximum seconds before verification fetch",
    )
    ...
    if args.verify_delay_min > args.verify_delay_max:
        parser.error("--verify-delay-min cannot be greater than --verify-delay-max")
...
def has_alert(snapshot: Dict[str, object]) -> bool:
    return bool(
        snapshot["increases"]
        or snapshot["new_listed_products"]
        or snapshot["watched_in_stock_events"]
        or snapshot["in_stock_count_increase"]
    )
...
def collect_snapshot(
    ...
    previous_in_stock_count: Optional[int],
    ...
    fake_new = [
        {
            ...
            "current_stock": 2,
            "detail_url": "https://www.firstcry.com/search?query=87654321",
            "search_url": "https://www.firstcry.com/search?query=87654321",
        }
    ]
